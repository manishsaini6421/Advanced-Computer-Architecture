...
> "$RAW_FILE"
echo "Matrix size,Avg L1-dcache-loads,Avg L1-dcache-load-misses,Avg Execution Time(ms),Avg MPKI" > "$AVG_FILE"
...
parse_output() {
    ...
    exec_time=$(echo "$output" | grep "SIMD matrix multiplication took" | awk '{print $5}')
...
for size in "${MATRIX_SIZES[@]}"; do
    ...
    echo "Matrix size,L1-dcache-loads,L1-dcache-load-misses,Execution Time(ms),MPKI" >> "$RAW_FILE"
    ...
    for ((i=1; i<=10; i++)); do
        output=$(perf stat -e L1-dcache-loads,L1-dcache-load-misses \
        ...
        if [ $? -eq 0 ]; then
            ...
            if [ "$l" -gt 0 ]; then
                mpki=$(echo "scale=3; ($m * 1000) / $l" | bc)
            else
                mpki=0
            fi

            echo "$size,$l,$m,$t,$mpki" >> "$RAW_FILE"
    ...
    echo "$size,Avg,$avg_loads,$avg_misses,$avg_time,$avg_mpki" >> "$RAW_FILE"
    echo "$size,$avg_loads,$avg_misses,$avg_time,$avg_mpki" >> "$AVG_FILE"

    # Print live progress
    echo "✅ Matrix size $size done → Avg Loads=$avg_loads, Avg Misses=$avg_misses, Time=$avg_time ms, MPKI=$avg_mpki"
done

echo ""
echo "Results written to:"
echo "  - $RAW_FILE (raw runs + averages per block, with MPKI per run, no run number)"
